...
function get_iphorm_phone(obj,e){
	...
	if($pre_element.find(".intl-tel-input").length>0){
		var input = $pre_element.find("input")[0];
		var iti = window.intlTelInputGlobals.getInstance(input);
		var country_data = iti.getSelectedCountryData();
		if(phone.indexOf("+")<0){
			phone = "+"+country_data.dialCode+phone;
		}
		if (!iti.isValidNumber()) {
			var errorCode = iti.getValidationError();
			$error = '<div class="iphorm-errors-list iphorm-clearfix"><div class="iphorm-error"><i class="fa fa-exclamation-triangle" aria-hidden="true" style="padding-right:5px;"></i>'+itel_phone_error+'</div></div>';
...
;(function($) {
	...
	jQuery(function($){
		var selection = $("[name='iphorm_sms_phone_id']");

		if(selection.length>0 && window.intlTelInput){
			var prev = selection.closest(".iphorm-element-wrap").prev();

			var input = prev.find("input.iphorm-element-text")[0];
			var name =  prev.find("input.iphorm-element-text").attr("name");
			if(input){
				var v = input.value;
				var iti = window.intlTelInput(input, {
					...
					initialCountry: itel_phone_initialCountry,
					onlyCountries: itel_phone_onlyCountries,
					utilsScript: "/qfy-content/plugins/bitcommerce/assets/js/frontend/wc-utils.js",
				});
				if(v){
					prev.find.find("[name='"+name+"']").val(v);
				}

				var preventAlert = true;

				var reset = function() {
					jQuery(input).css("outline","").attr("title","");
					preventAlert = true;
				};
				...
				input.addEventListener('blur', function() {
					...
					if (input.value.trim()) {
						if (iti.isValidNumber()) {
							prev.find('.qfyuser-warning').remove();
						} else {
							var errorCode = iti.getValidationError();
							$error = '<div class="iphorm-errors-list iphorm-clearfix"><div class="iphorm-error"><i class="fa fa-exclamation-triangle" aria-hidden="true" style="padding-right:5px;"></i>'+itel_phone_error+'</div></div>';
							$('.iphorm-errors-wrap', prev).html($error).fadeIn(1000).show();
						}
					}
				});
			}
			 ...
			 if(typeof jQuery.fn.ajaxSubmit=="undefined"){
				 jQuery.getScript( "/qfy-content/plugins/qfy_form/js/jquery.form.min.js",function(){
				 	if(_this.$form.find(".g-recaptcha-response").length>0 && grecaptcha) {
						_this.re_submit();
					}else if( _this.$form.find(".captcha-card-body").length>0 ){
						_this.$form.find(".captcha-card-body").show();
					}else{
						_this._submit();
					}
				 });
				...
				success: function(response) {
					...
					if (response === null || response === undefined) {
						...
						if (response.type == 'success') {
							...
							if(window.gtag_event_1){
								gtag_custom_report_conversion(window.gtag_event_1);
							}
